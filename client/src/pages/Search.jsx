    ...
    <form onSubmit={onSubmit} className="flex flex-col gap-5">
      ...
      <div>
        ...
        <div className="flex gap-2 items-center">
          <input type="number" id="minPrice" placeholder="Min"
            value={data.minPrice} onChange={onChange} className={inputCls} />
          <span className="text-slate-300 font-bold">—</span>
          <input type="number" id="maxPrice" placeholder="Max"
            value={data.maxPrice} onChange={onChange} className={inputCls} />
        </div>
      </div>
        ...
        <div className="flex gap-2">
          {[{ label: 'Any', val: '' }, { label: '1', val: '1' }, { label: '2', val: '2' }, { label: '3', val: '3' }, { label: '4+', val: '4' }].map(({ label, val }) => (
            <button key={label} type="button"
              onClick={() => onChange({ target: { id: 'bedrooms', value: val } })}
      ...
      <div className="flex gap-3 pt-1">
        <button type="button" onClick={onReset}
          className="flex-1 py-2.5 rounded-xl border-2 border-slate-200 text-slate-600 text-sm font-semibold hover:border-slate-400 transition">
          Reset
        </button>
        <button type="submit"
          className="flex-1 py-2.5 rounded-xl bg-green-600 hover:bg-green-700 text-white text-sm font-semibold transition shadow-sm">
          Apply
        </button>
      </div>
    </form>
...
export default function Search() {
  ...
  useEffect(() => {
    ...
    const parsed = {
      ...
      swimmingPool: urlParams.get('swimmingPool')  === 'true',
      gym:          urlParams.get('gym')           === 'true',
      playArea:     urlParams.get('playArea')      === 'true',
      garden:       urlParams.get('garden')        === 'true',
      security:     urlParams.get('security')      === 'true',
      wifi:         urlParams.get('wifi')          === 'true',
      powerBackup:  urlParams.get('powerBackup')   === 'true',
  ...
  const handleChange = (e, target = sidebardata, setter = setSidebardata) => {
    ...
    if (id === 'suitableFor') {
      setter(p => ({ ...p, suitableFor: value }));
      return;
    }
    if (id === 'searchTerm' || id === 'minPrice' || id === 'maxPrice' || id === 'bedrooms') {
      setter(p => ({ ...p, [id]: value }));
      return;
    }
    if (['parking','furnished','offer','swimmingPool','gym','playArea','garden','security','wifi','powerBackup'].includes(id)) {
      setter(p => ({ ...p, [id]: checked }));
      return;
    }
    if (id === 'sort_order') {
      const [sort, order] = value.split('_');
      setter(p => ({ ...p, sort: sort || 'createdAt', order: order || 'desc' }));
    }
  };

  // Drawer gets its own change handler using drawerData
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    navigate(`/search?${buildParams(sidebardata)}`);
  };
  ...
  const handleReset = () => {
    setSidebardata(DEFAULT_SIDEBAR);
    navigate('/search');
  };
    ...
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-3">
          <form onSubmit={handleSubmit} className="flex-1 flex items-center gap-2 bg-slate-100 rounded-xl px-4 py-2.5 border border-slate-200 focus-within:border-green-400 focus-within:bg-white transition">
            ...
            {sidebardata.searchTerm && (
              <button type="button"
                onClick={() => { setSidebardata(p => ({ ...p, searchTerm: '' })); }}
          ...
          <button
            type="button"
            onClick={() => { setDrawerData(sidebardata); setDrawerOpen(true); }}
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 flex gap-6">
        ...
        <main className="flex-1 min-w-0">
          ...
          {/* Your original show more button */}
          {!loading && showMore && (
            <div className="flex justify-center mt-8">
              <button onClick={onShowMoreClick}
                className="flex items-center gap-2 bg-white border-2 border-green-500 text-green-600 hover:bg-green-50 px-8 py-3 rounded-xl font-semibold text-sm transition shadow-sm">
      ...
      <div className={`fixed bottom-0 left-0 right-0 z-50 lg:hidden bg-white rounded-t-3xl shadow-2xl transition-transform duration-300 ease-in-out ${drawerOpen ? 'translate-y-0' : 'translate-y-full'}`}>
        <div className="flex items-center justify-between px-5 pt-4 pb-3 border-b border-slate-100">
          ...
          <button onClick={() => setDrawerOpen(false)} className="text-slate-400 hover:text-slate-600 transition">
            <FaTimes />
          </button>
        </div>
